...
const CustomLink = ({ href, children }) => (
  <a href={href} className="hover:text-orange-400 dark:hover:text-amber-600">{children}</a>
);
...
export default function Home() {
  ...
  return (
    <main className={`flex min-h-screen flex-col items-center justify-between ${styles.homeContainer} ${styles.blurEffect}`}>
      {/* Header */}
      <header className="bg-stone-700 w-full h-24 flex justify-between items-center mb-22">
        {/* Logo */}
        <div className="flex items-center ml-8">
          <img src="/logo/purenars.png" alt="Logo" className="h-20 w-auto" /> 
        </div>
        {/* Burger Menu Icon */}
        ...
        {/* Menu Items */}
        <div className={`mr-40 font-semibold text-gray-200 text-xl flex-row space-x-12 md:flex hidden ${isMenuOpen ? "flex" : "hidden"}`}>
        ...
        <div className="mr-24 text-white text-xl font-semibold gap-4 flex items-center space-x-6">
          ...
          <a href="/Signup" className="bg-amber-600 hover:bg-amber-700 text-white px-4 py-2 rounded-md transition-colors duration-300 ease-in-out">Sign Up</a>
        </div>
      </header>
       ...
       <div className="flex flex-col items-center text-black space-x-16">
        <div className="flex flex-row items-center justify-center w-full">
          ...
          Welcome to <span className="text-orange-400" style={{ textShadow: "1px 1px 1px black" }}>Nar's</span> <span className="text-white drop-shadow-[2px_3px_3px_var(--tw-shadow-color)] shadow-black">School Supplies</span>
            </h1>
          ...
          {/* Image */}
          <img src="/logo/bag.png" alt="School Supplies Image" className="h-46 w-auto ml-8 mt-6"  style={{ marginLeft: "12rem" }} /> {/* Replace 'your-image.jpg' with your actual image filename */}
        </div>
      </div>
